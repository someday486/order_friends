...
export default function BusinessBillingSetupPage() {
  ...
  const handleOpenBillingAuth = async () => {
    ...
    try {
      ...
      await payment.requestBillingAuth({
        ...
        successUrl: `${origin}/business/billing/setup?brandId=${encodeURIComponent(brandId)}`,
        failUrl: `${origin}/business/billing/setup?brandId=${encodeURIComponent(brandId)}`,
      });
    } catch (requestError) {
      setError(
        parseApiErrorMessage(
    ...
    <div className="mx-auto flex w-full max-w-3xl flex-col gap-6 px-4 py-6">
      <div>
        ...
        <p className="mt-2 text-[14px] leading-6 text-text-secondary">
          14일 무료 체험 후 월 ₩33,000부터 자동 결제됩니다. 카드 등록이 끝나면
          바로 브랜드 운영을 시작할 수 있어요.
        </p>
      </div>
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            <div className="mt-2 text-[13px] leading-6 text-text-secondary">
              토스 자동결제 등록창에서 카드를 등록하면 Starter 플랜 14일 무료 체험이
              바로 시작됩니다. 무료 체험이 끝나면 등록한 카드로 자동 결제가 진행됩니다.
            </div>
          </div>
        </div>
