...
function getPeriodStatusLabel(status: SettlementPeriodResponse['status'] | null | undefined) {
  switch (status) {
    case 'PENDING':
      return '집계 대기';
    case 'CALCULATED':
...
function getPeriodStatusClass(status: SettlementPeriodResponse['status'] | null | undefined) {
  switch (status) {
    case 'PENDING':
...
export default function BusinessSettlementPage() {
      ...
      {!brandsLoading && manageableBrands.length === 0 ? (
        <Card>
          <CardContent className="py-10 text-center text-[13px] text-text-secondary">
            ...
            <Link href="/business/billing" className="ml-1 font-semibold text-foreground">
              빌링 화면
            </Link>
            에서 확인할 수 있습니다.
          </CardContent>
                            ...
                            >
                              ...
                              <div className="text-[13px] text-text-secondary">
                                ...
                                <div className="mt-1 font-semibold text-foreground">
                                  {formatWon(item.saleAmount)} / {formatWon(item.commissionAmount)}
                                </div>
                              </div>
                              <div className="text-[13px] text-text-secondary">
