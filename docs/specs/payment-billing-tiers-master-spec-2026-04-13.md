...
현재 Order Friends는 온라인마켓 운영 플랫폼으로서 결제수단을 CARD(토스페이먼츠)와 TRANSFER(계좌이체)로 구분하고, 브랜드/스토어/출고지 단위로 허용 결제수단을 배열로 관리한다.

이를 **브랜드 가입 시점에 결정되는 과금 체계**로 전면 재구성한다.

- **PG 이용 고객** (PG 티어): 토스페이먼츠를 통해 카드 결제를 제공한다. PG 원가와 Order Friends 플랫폼 수수료는 수익성 관리 관점에서 분리 추적한다.
- **비이용 고객** (Non-PG 티어): 구매자가 브랜드/스토어의 계좌번호를 보고 직접 이체한다. 브랜드는 Order Friends에 월 구독료를 자동 결제한다.
...
3. TRANSFER: 주문 생성 후 브랜드/스토어 계좌정보 표시 → 고객이 수동 이체 → 입금 확인
...
- 현재 seed 데이터의 33,000/44,000/55,000원 플랜은 초기 실험값이며, 상용 가격 확정 전 조정 대상이다.
...
| # | 결정 | 근거 |
| --- | --- | --- |
| D1 | 결제 모드 선택 단위는 **브랜드** | 산하 모든 스토어/출고지에 동일 적용. 과금 체계 단순화 |
...
| D4 | CASH(현장 결제) **완전 폐지** | 온라인마켓 주문 플랫폼으로 현금 결제 불필요 |
...
| D7 | PG 수수료율은 **PG 원가와 플랫폼 수익을 분리해 관리** | `commission_tiers`는 운영 수수료 정책 실험값으로 취급 |
| D8 | `deposit_match_rows` 시스템 **Non-PG 티어에서 유지** | 입금 확인 워크플로우 그대로 사용 |
| D9 | 기존 브랜드는 **전부 PG로 배정** | Non-PG는 신규 가입자부터만 적용. 기존 운영 중단 방지 |
| D10 | 테이블오더는 제품 포지션에서 제외 | Branch는 사용자 화면에서 스토어/출고지/판매 채널로 표현 |
...
2. PG 티어 고객에게는 토스페이먼츠 기반 카드 결제 경험을 노출한다.
3. Non-PG 티어 고객에게는 계좌이체라는 단순한 경험을 유지한다.
4. 과금(수수료/구독료)은 플랫폼의 수익 모델이다. PG 원가, 플랫폼 수익, 정산액을 분리해 투명하게 추적한다.
5. 매출 규모가 커지는 브랜드에게는 별도 계약 또는 플랜 기반 인센티브를 제공한다.
6. 정산과 빌링은 감사 가능한 레코드를 남긴다.
7. 공동구매는 현재 주력 포지션이 아니라 향후 캠페인 판매 기능으로 검토한다.
