...
describe('CustomerProductsService', () => {
  ...
  it('checkProductAccess should throw when product missing', async () => {
    chains.products.single.mockResolvedValueOnce({
      data: null,
  ...
  it('checkModificationPermission should allow OWNER', () => {
    expect(() =>
      (service as any).checkModificationPermission('OWNER', 'update', 'u1'),
    ).not.toThrow();
  });
  ...
  it('getMyCategories should map defaults for nullable fields', async () => {
    chains.branches.single.mockResolvedValueOnce({
      data: { id: 'b1', brand_id: 'brand-1' },
      error: null,
    });
  ...
  it('getMyProducts should throw when fallback also fails', async () => {
    chains.branches.single.mockResolvedValueOnce({
      data: { id: 'b1', brand_id: 'brand-1' },
      error: null,
    });
  ...
  it('getMyProduct should throw when product missing', async () => {
    chains.products.single.mockResolvedValueOnce({
      data: null,
      error: { message: 'missing' },
    });
  ...
  it('createMyProduct should create product without options', async () => {
    ...
    expect(chains.products.insert).toHaveBeenCalledWith(
      expect.objectContaining({ is_hidden: true }),
    );
    expect(chains.product_options.insert).not.toHaveBeenCalled();
  });
  ...
  it('updateMyProduct should throw when role is not allowed', async () => {
    chains.products.single.mockResolvedValueOnce({
      data: { id: 'p1', branch_id: 'b1', branches: { brand_id: 'brand-1' } },
      error: null,
    });
  ...
  it('updateMyProduct should map isActive to is_hidden', async () => {
    chains.products.single
      ...
      .mockResolvedValueOnce({
        data: { id: 'p1', name: 'Updated', is_hidden: true },
        error: null,
      });

    await service.updateMyProduct(
      'u1',
      'p1',
      { isActive: false, sortOrder: 2 } as any,
      [],
      [{ branch_id: 'b1', role: 'OWNER' }],
    );

    expect(chains.products.update).toHaveBeenCalledWith(
      expect.objectContaining({ is_hidden: true, sort_order: 2 }),
    );
  });
  ...
  it('deleteMyProduct should throw when role is not allowed', async () => {
    chains.products.single.mockResolvedValueOnce({
      data: { id: 'p1', branch_id: 'b1', branches: { brand_id: 'brand-1' } },
      error: null,
    });
  ...
  it('reorderProducts should throw when role missing', async () => {
    chains.branches.single.mockResolvedValueOnce({
      data: { id: 'b1', brand_id: 'brand-1' },
      error: null,
    });
    ...
    await expect(
      service.reorderProducts('u1', 'b1', [{ id: 'p1', sortOrder: 1 }], [], [
        { branch_id: 'b1', role: undefined },
      ] as any),
    ).rejects.toThrow(ForbiddenException);
  });

  it('reorderProducts should update items and return list', async () => {
    chains.branches.single.mockResolvedValueOnce({
      data: { id: 'b1', brand_id: 'brand-1' },
      error: null,
    });
  ...
  it('updateCategory should throw when category missing', async () => {
    ...
    await expect(
      service.updateCategory('u1', 'c1', { name: 'New' }, [], []),
    ).rejects.toThrow(NotFoundException);
  });

  it('updateCategory should update when fields provided', async () => {
    ...
    const result = await service.updateCategory(
      'u1',
      'c1',
      { name: 'New', sortOrder: 2, isActive: false },
      [],
      [{ branch_id: 'b1', role: 'OWNER' }],
    );
