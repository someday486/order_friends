...
export class PublicOrderService {
   ...
   * Get order (public)
   */
  async cancelPublicOrder(orderId: string): Promise<PublicOrderResponse> {
    const adminSb = this.supabase.adminClient();

    const { data: order, error } = await adminSb
      .from('orders')
      .select('id, branch_id, order_no, status')
      .eq('id', orderId)
      .maybeSingle();
    ...
    if (!this.paymentsService) {
      throw new Error('PaymentsService is not available.');
    }
